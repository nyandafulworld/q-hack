...
$(document).ready(function () {
  $('..header-conversion').on('click',function(){
    var $header = jQuery('.js-header');
    if ($header.hasClass('open')) {
    $header.removeClass('open');
    fixedBase('close');
...
$(function () {
    ...
    const showPopup = () => {
        $popup.show();
        $popupContent.addClass('pop-show');
    };
    ...
    const checkScroll = () => {
        ...
        console.log('scrollRatio:', ratio.toFixed(3));

        // スクロール率が3%以上になったら1回だけ表示
        if (!hasShownPopup && !isPopupClosed && ratio >= 0.2) {
            showPopup();
            hasShownPopup = true;
        }
    };
    ...
    $closeBtn.on('click', function () {
        $popup.hide();
        $popupContent.removeClass('pop-show');
        isPopupClosed = true;
    });
});
